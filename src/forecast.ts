...
export async function getWeatherData(lat: number, lon: number, unit: string) {
  const params = {
    latitude: lat,
    longitude: lon,
    hourly: [
  ...
  return {
    hourly: {
      ...
      wind_speed: Array.from(hourly.variables(4)?.valuesArray() ?? []).map(Math.round),
      wind_direction: Array.from(hourly.variables(5)?.valuesArray() ?? []).map(getCardinalDirection),
    },
    daily: {
      time: [
        ...Array(
